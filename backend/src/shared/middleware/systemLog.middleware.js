  ...
  res.on('finish', async () => {
    if (res.statusCode >= 200 && res.statusCode < 400 && req.user) {
      try {
        ...
        if (req.originalUrl.includes('/register-hotels'))
          action = 'Created Hotel Request';
        else if (req.originalUrl.includes('/login')) action = 'User Logged In';
        else if (req.originalUrl.includes('/bookings') && req.method === 'POST')
          action = 'Created Booking';
        else if (req.originalUrl.includes('/approvals'))
          action = 'Resolved Approval';
